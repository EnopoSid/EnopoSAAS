...
@Component({
  selector: 'app-addOurBrands',
  templateUrl: './addOurBrands.component.html',
  encapsulation: ViewEncapsulation.None
})
export class AddOurBrandsComponent implements OnInit {
  updateText:any;
  ...
  ngOnInit() {
    ...
    if (isNaN(this.id)) {
      this.title = "Add Sponsors"
    }
    if (!!this.id) {
      this.sendReceiveService.getKeyValueName(this.id).subscribe(resp => {
        if (!!this.service.isOurBrandView) {
          this.title = "View Sponsors - " + this.id;
        }
        else {
          this.title = "Edit Sponsors - " + this.id;
        }
      })
    }
    ...
    if (!isNaN(this.id)) {
      ...
      this.service.getBrandListById(this.id).subscribe(resp => {
  ...
  onAddBrandSubmit() {
    ...
    let var_id: number = this.AddBrand.value.id;
    let var_titleName: string = this.AddBrand.value.titleName;
    let var_subTitle: string = this.AddBrand.value.subTitle;
    let var_shortDescription: string = this.AddBrand.value.shortDescription;
    let var_fullDescription: string = this.AddBrand.value.fullDescription;
    let var_image = this.selectedFiles;
    
    if (this.saveUser) {
     
      this.service.saveBrandList({
        "ModifiedDate": null,
        "CreatedDate": null,
        ...
        this.editImage ="";
        this.router.navigate(['/landingpage/ourBrands']);
      
      },
        error => this.addBrandFormErrors = error
      )
    }
