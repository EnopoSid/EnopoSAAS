...
import { StoreTimingsService } from './store-timings.service';
...
export class StoreTimingsComponent implements OnInit {
    ...
    constructor(private router: Router,
        ...
        public service: StoreTimingsService,
...
ngOnInit(){
    ...
    this.ConfigurationForm = this.formBuilder.group({
        id: 0,
        'Value':  [null, Validators.compose([Validators.required, Validators.minLength(3),,Validators.pattern('^[A-Z\d]{2}(?::[A-Z\d]{2}){5}$')])],
    });
    ...
    if (this.title == "Save") { 
        ...
        this.service.updateConfiguration({
            ...
            .subscribe((data) => {
               ...
               this.ConfigurationForm = this.formBuilder.group({
                id: 0,
                'Value':  [null, Validators.compose([Validators.required,,Validators.pattern(MyAppHttp.HTML_VALIDATION_PATTERNS.onlyNumbers)])],
            });
               this.getAllConfigurations();
               this.ConfigurationForm.reset();
            }, error => {
...
activateRecord(event, id, orderid) {
  ...
  dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
          this.service.ActivateStatus(id, orderid).subscribe((data) => {
              this.getAllConfigurations();
          }, error => {
