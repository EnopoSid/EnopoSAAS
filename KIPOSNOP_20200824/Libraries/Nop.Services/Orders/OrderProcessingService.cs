...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual PlaceOrderContainer PreparePlaceOrderDetailsForOnline(ProcessPaymentRequest processPaymentRequest, List<ShoppingCartItem> cart, decimal orderTax)
        {
            ...
            decimal categoryDiscountAmount = 0;
            decimal categoryDiscountAmountTax = 0;

            foreach(var cartItem in details.Cart)
            {
                decimal taxRate = decimal.Zero;
                decimal shoppingCartItemSubTotalWithDiscountBaseInclTax = _taxService.GetProductPrice(cartItem.Product, _priceCalculationService.GetSubTotal(cartItem, true), out taxRate);
                decimal shoppingCartItemSubTotalWithDiscountInclTax = shoppingCartItemSubTotalWithDiscountBaseInclTax;

                //Added by sree for inclusion of tax 07_18_2020 start
                decimal shoppingCartItemSubTotalWithDiscountBaseExclTax = _taxService.GetProductPrice(cartItem.Product, _priceCalculationService.GetSubTotal(cartItem, true), false, details.Customer, out taxRate);
                
                //display an applied discount amount
                decimal shoppingCartItemSubTotalWithoutDiscountBaseInclTax = _taxService.GetProductPrice(cartItem.Product, _priceCalculationService.GetSubTotal(cartItem, false), out taxRate);

                decimal shoppingCartItemSubTotalWithoutDiscountBaseExclTax = _taxService.GetProductPrice(cartItem.Product, _priceCalculationService.GetSubTotal(cartItem, false), false, details.Customer, out taxRate);
                ...
                //Added by sree for inclusion of tax 07_18_2020 end
                decimal shoppingCartItemDiscountBase = shoppingCartItemSubTotalWithoutDiscountBaseExclTax - shoppingCartItemSubTotalWithDiscountBaseExclTax;
                if (shoppingCartItemDiscountBase > decimal.Zero)
                {
				//Modified  By sree for category discount 19_08_2020 start

                    _taxService.GetTaxRate(details.Cart[0].Product, details.Cart[0].Product.TaxCategoryId, details.Customer, details.Cart[0].Product.Price, out var newTax, out _);
                    //Added by sree for inclusion of tax 07_18_2020 start
                    categoryDiscountAmount = categoryDiscountAmount+shoppingCartItemDiscountBase;
				//Modified  By sree for category discount 19_08_2020 start
                    //Added by sree for inclusion of tax 07_18_2020 end
                    categoryDiscountAmountTax = (categoryDiscountAmount * newTax) / 100;
                }
            }
            //Added for category discount by sree 16_06_2020 end


            //Added By Sree for Redeem Amount based on total payable amount  07-06-2020 start
            decimal ActualDiscountAmountExclTax = 0;
            //Added By sree for inclusion of tax 07-17-2020 start
            decimal ActualDiscountAmountInclTax = 0;
            foreach (var disc in orderSubTotalAppliedDiscounts)
                if (!_discountService.ContainsDiscount(details.AppliedDiscounts, disc))
                {
                    if (disc.CouponCode.ToLower().Contains("kp_"))
                    {
                        details.RedeemedRewardPointsAmount = disc.DiscountAmount;
                    }
                    else
                    {
                        //Added By sree to for fixed amount discount requirement 16_08_2020 start
                        if (disc.UsePercentage==false)
                        {
                            decimal disscountTotal = disc.DiscountAmount;
                            if (disc.DiscountAmount>=details.OrderSubTotalDiscountInclTax)
                            {
                                disscountTotal = details.OrderSubTotalDiscountInclTax;
                            }
                            else
                            {
                                disscountTotal = disc.DiscountAmount;
                                
                            }
                            ActualDiscountAmountExclTax = ActualDiscountAmountExclTax + disscountTotal;
                            //Added By sree for inclusion of tax 07-17-2020 start
                            ActualDiscountAmountInclTax = ActualDiscountAmountInclTax + disscountTotal;
                        }
                        //Added By sree to for fixed amount discount requirement 16_08_2020 end
                        else
                        {
                            decimal discountTotal = (details.OrderSubTotalExclTax) * (disc.DiscountPercentage) / 100;
                            decimal discountTotalIncl = (details.OrderSubTotalInclTax) * (disc.DiscountPercentage) / 100;
							//Added  By sree for maxdiscount amount condition 19_08_2020 start
                            if (disc.MaximumDiscountAmount>0)
                            {
                                if (discountTotalIncl>= disc.MaximumDiscountAmount)
                                {
                                    var discountamountLocal= Convert.ToDecimal(disc.MaximumDiscountAmount);
                                    discountTotalIncl = discountamountLocal;
                                    _taxService.GetTaxRate(details.Cart[0].Product, details.Cart[0].Product.TaxCategoryId, details.Customer, details.Cart[0].Product.Price, out var tax, out _);
                                    discountTotal = CalculateTaxPrice(discountamountLocal, tax);
                                }
                            }
                            //Added  By sree for maxdiscount amount condition 19_08_2020 end
                            if (discountTotal >= details.OrderSubTotalDiscountInclTax)
                            {
                                discountTotal = details.OrderSubTotalDiscountInclTax;
                            }
                            ActualDiscountAmountExclTax = ActualDiscountAmountExclTax + discountTotal;
                            ActualDiscountAmountInclTax = ActualDiscountAmountInclTax + discountTotalIncl;
                        }
                    }
                    details.AppliedDiscounts.Add(disc);
                }
            //Added By Sree for Redeem Amount based on total payable amount  07-06-2020 end
            ...
            {
                details.OrderSubTotalExclTax = details.OrderSubTotalExclTax + categoryDiscountAmount;
                details.OrderSubTotalDiscountExclTax = details.OrderSubTotalDiscountExclTax + categoryDiscountAmount;
                details.OrderSubTotalInclTax = details.OrderSubTotalInclTax + categoryDiscountAmount+ categoryDiscountAmountTax;
                details.OrderSubTotalDiscountInclTax = details.OrderSubTotalDiscountInclTax + categoryDiscountAmount + categoryDiscountAmountTax;
            }
            //Added for category discount by sree 16_06_2020 end
            ...
            if (_shoppingCartService.ShoppingCartRequiresShipping(details.Cart))
            {
                ...
                if (_shippingSettings.AllowPickUpInStore && pickupPoint != null)
                    {
                        var country = _countryService.GetCountryByTwoLetterIsoCode(pickupPoint.Address);
            ...
            if (details.RedeemedRewardPointsAmount != 0)
            {
                //Added for category discount by sree 16_06_2020 start

                    ActualDiscountAmountExclTax = ActualDiscountAmountExclTax + categoryDiscountAmount;
                //ActualDiscountAmountInclTax = ActualDiscountAmountExclTax + categoryDiscountAmount + categoryDiscountAmountTax;



                //Added for category discount by sree 16_06_2020 end
                //_orderTotalCalculationService.GetShoppingCartSubTotal(details.Cart, false, out _, out _, out _, out _, out var orderSubTotalTaxRates);
                _taxService.GetTaxRate(details.Cart[0].Product, details.Cart[0].Product.TaxCategoryId, details.Customer, details.Cart[0].Product.Price, out var taxRate, out _);
                ...
                decimal tempActualAmountInclTax = 0;
                if (ActualDiscountAmountExclTax == ActualDiscountAmountInclTax)
                {
                    tempActualAmountInclTax = ActualDiscountAmountExclTax;
                    ActualDiscountAmountExclTax = ActualDiscountAmountInclTax / (1 + (taxRate / 100));
                    ActualDiscountAmountExclTax= Math.Round(ActualDiscountAmountExclTax, 2);
                }
                else
                {
                    tempActualAmountInclTax = (ActualDiscountAmountExclTax) * (taxRate) / 100;
                    tempActualAmountInclTax = ActualDiscountAmountExclTax + tempActualAmountInclTax;
                }


                decimal taxCalulationCustomForZeroPayableAmount = details.OrderSubTotalInclTax - tempActualAmountInclTax - details.RedeemedRewardPointsAmount + details.OrderShippingTotalInclTax;


                
                string[] TaxkeysForZeroPayableAmount = details.TaxRates.Split(':');

                if(taxCalulationCustomForZeroPayableAmount==0)
                {
                    details.OrderTaxTotal = 0;
                    details.OrderTaxTotal = Math.Round(details.OrderTaxTotal, 2);
                }
                else
                {
                    details.OrderTaxTotal = 0;
                    decimal tempOrderToatalTax = (taxCalulationCustomForZeroPayableAmount) * (taxRate) / (100 + taxRate);
                    details.OrderTaxTotal = details.OrderTaxTotal + tempOrderToatalTax;
                    details.OrderTaxTotal = Math.Round(details.OrderTaxTotal, 2);
                }

                details.OrderSubTotalDiscountExclTax = Math.Round(ActualDiscountAmountExclTax, 2);
                details.OrderSubTotalDiscountInclTax = Math.Round(tempActualAmountInclTax, 2);
                if (categoryDiscountAmount > 0)
                {
                    details.OrderSubTotalDiscountInclTax = categoryDiscountAmount + categoryDiscountAmountTax;
                }
                details.TaxRates = taxRate + ":" + details.OrderTaxTotal;
            }
            //Added By sree for inclusion of tax in fixed discount in online 14-08-2020
            else
            {
                decimal tempdisc = 0;
                foreach (var disc in details.AppliedDiscounts)
                    if (!disc.UsePercentage)
                    {
                        decimal disscountTotal = 0;
                        if (disc.DiscountAmount >= details.OrderSubTotalDiscountInclTax)
                        {
                            disscountTotal = details.OrderSubTotalDiscountInclTax;
                        }
                        ...
                        tempdisc = tempdisc + disscountTotal;
                    }
                    else
                    {
                        decimal disscountTotal = (details.OrderSubTotalInclTax) * (disc.DiscountPercentage) / 100;
						//Added  By sree for maxdiscount amount condition 19_08_2020 start
                        if (disc.MaximumDiscountAmount>0)
                        {
                            if (disscountTotal>= disc.MaximumDiscountAmount)
                            {
                                disscountTotal = Convert.ToDecimal(disc.MaximumDiscountAmount);
                            }
                        }
						//Added  By sree for maxdiscount amount condition 19_08_2020 end
                        if (disscountTotal >= details.OrderSubTotalDiscountInclTax)
                        {
                            disscountTotal = details.OrderSubTotalDiscountInclTax;
                        }
                        tempdisc = tempdisc + disscountTotal;
                ...
                if (categoryDiscountAmount>0)
                {
                    details.OrderSubTotalDiscountInclTax = categoryDiscountAmount + categoryDiscountAmountTax+tempdisc;
                }
                else
                {
                    details.OrderSubTotalDiscountInclTax = tempdisc;
                }
				//Added  By sree for category discount amount issue 19_08_2020 start
                if (details.OrderSubTotalDiscountExclTax == details.OrderSubTotalDiscountInclTax)
                {
                    _taxService.GetTaxRate(details.Cart[0].Product, details.Cart[0].Product.TaxCategoryId, details.Customer, details.Cart[0].Product.Price, out var newTax, out _);
                    
                    ActualDiscountAmountExclTax = CalculateTaxPrice(ActualDiscountAmountInclTax,newTax);
                    details.OrderSubTotalDiscountExclTax = ActualDiscountAmountExclTax;
                }
            }
				//Added  By sree for category discount amount issue 19_08_2020 end
            //Added By sree for inclusion of tax in fixed discount in online 14-08-2020
            ...
            details.OrderTaxTotal = orderTax;
            //details.OrderTotal=details.OrderDiscountAmount-
            //*For Redeem Amount by sree 09_02_2019*/
            details.OrderTotal = orderTotal.Value;
            /*For Redeem Amount by sree 09_02_2019*/

            //Added by sree for inclusion of tax 07_18_2020 start
            details.OrderTotal = details.OrderSubTotalInclTax - details.OrderSubTotalDiscountInclTax - details.RedeemedRewardPointsAmount + details.OrderShippingTotalInclTax;
            //Added by sree for inclusion of tax 07_18_2020 end

            //Added By Sree for ZeroPayable Amount by KPOINTS 04-06-2020 start
            if ((string)processPaymentRequest.CustomValues.Where(x => x.Key == "PaymentMethodType").FirstOrDefault().Value == "KPOINTs"
                //Added By Sree for PaymentMethod By DiscountCode 14-08-2020
                || (string)processPaymentRequest.CustomValues.Where(x => x.Key == "PaymentMethodType").FirstOrDefault().Value == "DiscountCode")
            {
                details.OrderTotal = 0;
            }
            //Added By Sree for ZeroPayable Amount by KPOINTS 04-06-2020 end

            //Added By surakshith for tax rate to be the acutal rate instead of 0% when order amount is zero 16_08_2020 start
            //_orderTotalCalculationService.GetShoppingCartSubTotal(details.Cart, false, out _, out _, out _, out _, out var orderSubTotalTaxRatesNew);
            _taxService.GetTaxRate(details.Cart[0].Product, details.Cart[0].Product.TaxCategoryId, details.Customer, details.Cart[0].Product.Price, out var taxRateNew, out _);

            //decimal taxRateNew = 0;
            //foreach (var kvp in orderSubTotalTaxRatesNew)
            //{
            //    taxRateNew = kvp.Key;
            //}
            details.TaxRates = taxRateNew + ":" + details.OrderTaxTotal;
            //Added By surakshith for tax rate to be the acutal rate instead of 0% when order amount is zero 16_08_2020 end
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Details</returns>
        protected virtual PlaceOrderContainer PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest, List<ShoppingCartItem> cart)
        {
            var details = new PlaceOrderContainer
            {
                //customer
            ...
            #region Commented By Praveen - As we are not taking the cart items from the customer object
            //load shopping cart
            ...
            foreach (var cartItem in details.Cart)
            {
                ...
                if (shoppingCartItemDiscountBase > decimal.Zero)
                {
                    ...
                    categoryDiscountAmount = categoryDiscountAmount + shoppingCartItemDiscountBase;
                    //Added by sree for inclusion of tax 07_18_2020 end
                    categoryDiscountAmountTax = (categoryDiscountAmount * taxRate) / 100;
                }
            }
            //Added for category discount by sree 16_06_2020 end
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            var order = new Order
            {
                ...
                PaidDateUtc = null,
                BillingAddress = details.PickUpInStore ? details.PickupAddress : details.ShippingAddress,
                ...
                DeliveryDate = Convert.ToDateTime (processPaymentRequest.CheckoutAddressDetails.DeliveryDate),
                /*For Redeem Amount by sree 09_02_2019*/
                RedeemAmount = details.RedeemedRewardPointsAmount,
                /*For Redeem Amount by sree 09_02_2019*/
                //Added By Sree for ZeroPayable Amount by KPOINTS 04-06-2020 start
                PaymentMethodType = (string)processPaymentRequest.CustomValues.Where(x => x.Key == "PaymentMethodType").FirstOrDefault().Value
                //Added By Sree for ZeroPayable Amount by KPOINTS 04-06-2020 end
            };
        ...
        /// </summary>
        /// <param name="order">Order</param>
        protected virtual void SendNotificationsAndSaveNotesForOnline(Order order,string MemberId,bool IsMember)
        {
            ...
            //Added By Surakshith to display memberId in mail for register user strat on 13-06-2020
            var orderPlacedStoreOwnerNotificationQueuedEmailIds = _workflowMessageService.SendOrderPlacedStoreOwnerNotificationForOnline(memberId,isMember,order, _localizationSettings.DefaultAdminLanguageId);
            //Added By Surakshith to display memberId in mail for register user end on 13-06-2020
            ...
            //Added By Surakshith to display memberId in mail for register user strat on 13-06-2020
            var orderPlacedCustomerNotificationQueuedEmailIds = _workflowMessageService
                .SendOrderPlacedCustomerNotificationForOnline(memberId,isMember,order, order.CustomerLanguageId, orderPlacedAttachmentFilePath, orderPlacedAttachmentFileName);
            //Added By Surakshith to display memberId in mail for register user end on 13-06-2020
        ...
        /// <param name="order">Order</param>
        protected virtual void MoveShoppingCartItemsToOrderItems(PlaceOrderContainer details, Order order, int ParentCategoryId = 0)
        {
            foreach (var sc in details.Cart)
            {
                ...
                foreach (var disc in scDiscounts)
                    if (!_discountService.ContainsDiscount(details.AppliedDiscounts, disc))
                        details.AppliedDiscounts.Add(disc);
                if (details.AppliedDiscounts.Count > 0)
                {
                    if (scSubTotal == 0)
                    {
                        var price = _priceCalculationService.GetSubTotal(sc, false);
                        discountAmountInclTax = price;
                        discountAmountExclTax = price;
                    }
                    scUnitPriceInclTax = scUnitPriceInclTax + discountAmountInclTax;
                    scUnitPriceExclTax = scUnitPriceExclTax + discountAmountExclTax;
                }
                ...
                var attributeDescriptionEmail =
                    _productAttributeFormatter.FormatAttributesEmail(sc.Product, sc.AttributesXml, details.Customer);
                ...
                var orderItem = new OrderItem
                {
                    ...
                    AttributeDescriptionEmail = attributeDescriptionEmail,
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest, List<ShoppingCartItem> cart)
        {
            ...
            try
            {
                ...
                /*Modified by sree for applying discounts to only one order during common checkout 24_01_2019*/
                var details = PreparePlaceOrderDetails(processPaymentRequest, cart);
                /*Modified by sree for applying discounts to only one order during common checkout 24_01_2019*/

                ProcessPaymentResult processPaymentResult = new ProcessPaymentResult();
                Boolean isSecondCheckount = false;
                if ((int)processPaymentRequest.CustomValues.Where(x=>x.Key=="CheckoutOrderCount").SingleOrDefault().Value == 1) { 
                processPaymentResult = GetProcessPaymentResult(processPaymentRequest, details);
                }
                else
                {
                    processPaymentResult.NewPaymentStatus = PaymentStatus.Paid;
                    processPaymentResult.CaptureTransactionId = processPaymentRequest.CustomValues.Where(x => x.Key == "CaptureTransactionId").SingleOrDefault().Value.ToString();
                    //processPaymentResult.CaptureTransactionResult = captureResult;
                   isSecondCheckount = true;
                }
               
                    if (processPaymentRequest.CheckoutAddressDetails.PickUpInStore == true)
                    {
                        details.OrderTotal = details.OrderTotal - details.OrderShippingTotalInclTax;
                ...
                if (!processPaymentRequest.CheckoutAddressDetails.PickUpInStore) {
                    details.ShippingAddress = _addressService.GetAddressById(processPaymentRequest.CheckoutAddressDetails.ShippingAddressId);
                    details.PickUpInStore = false;
                    // added by Phani for adding cc mails for only online orders on 6/04/2020 start
                    if (processPaymentRequest.PaymentMethodSystemName == "Stripe.Cards")
                    {
                        IList<PickupPoint> pickupPoints = _shippingService.GetPickupPoints(null).PickupPoints;
                        foreach (var pickupPoint in pickupPoints)
                        {
                                processPaymentRequest.CustomValues.Add("CCAddress", "del_" + cart[0].Customer.Addresses[0].Email + ";" + pickupPoint.CCAddress);
                                break;
                        }
                    }
                    // added by Phani for adding cc mails for only online orders on 6/04/2020 end
                }
                else
                {
                    details.PickupAddress = _addressService.GetAddressById(processPaymentRequest.CheckoutAddressDetails.PickupAddressId);
                    details.PickUpInStore = true;
                    /*added by sree for getting pickupinstore address 07_01_2019 Start*/
                    IList<PickupPoint> pickupPoints = _shippingService.GetPickupPoints(null).PickupPoints;
                    foreach (var pickupPoint in pickupPoints)
                    {
                        if (processPaymentRequest.CheckoutAddressDetails.PickupAddressId == Convert.ToInt32(pickupPoint.Id))
                        {
                            details.PickUpInStore = true;
                            var country = _countryService.GetCountryByTwoLetterIsoCode(pickupPoint.Address);
                            var state = _stateProvinceService.GetStateProvinceByAbbreviation(pickupPoint.StateAbbreviation, country?.Id);
                            ...
                            /*added by sree for getting email and name while pickup 23_01_2019 End*/
                            details.PickupAddress.Email = details.Customer.Email;
                            details.PickupAddress.FirstName = details.Customer.Username;
                            // added by Phani for adding cc mails for only online orders on 6/04/2020 start
                            details.PickupAddress.LastName = pickupPoint.ContactAddress;
                           // }
                            // added by Phani for adding cc mails for only online orders on 6/04/2020 end
                ...
                if (processPaymentResult.Success || isSecondCheckount)
                {
                    ...
                    if (processPaymentRequest.CheckoutAddressDetails.PickUpInStore == true)
                    {
                        Order_Pickup_CustDetails order_Pickup_CustDetails = new Order_Pickup_CustDetails
                        {
                            MobileNumber = details.Customer.MobileNumber,
                            CustomerId = details.Customer.Id,
                            OrderId = order.Id,
                            CreatedDate = DateTime.Now,
                            ModifiedDate = DateTime.Now,
                            EmailId = details.Customer.Email,
                            IsActive = true
                        };
                        _order_Pickup_CustDetailsService.InsertPickUpDetails(order_Pickup_CustDetails);
                    }

                    //move shopping cart items to order items
                    MoveShoppingCartItemsToOrderItems(details, order, cart[0].ParentCategoryId);
                    ...
                    // added by Phani for adding cc mails for only online orders on 6/04/2020 start
                    if (processPaymentRequest.PaymentMethodSystemName == "Stripe.Cards" && details.PickupAddress == null)
                    {
                        order.BillingAddress.Company = processPaymentRequest.CustomValues.Where(x => x.Key == "CCAddress").SingleOrDefault().Value.ToString();
                    }
                    // added by Phani for adding cc mails for only online orders on 6/04/2020 end
                    //notifications
                    ...
                    #region Commented By Praveen - Added directly after Successful checkout of all Orders (Checkout Controller - ConfirmOrder Method)
                    //reset checkout data
                    ...
                    //check order status
                    CheckOrderStatusOrderProcess(order,cart[0].ParentCategoryId);
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult PlaceOrderForOnline(ProcessPaymentRequest processPaymentRequest, List<ShoppingCartItem> cart, string MemberId,bool IsMember,decimal orderTax)
        {
            if (processPaymentRequest == null)
                throw new ArgumentNullException(nameof(processPaymentRequest));
            ...
            try
            {
                ...
                //prepare order details
                /*Modified by sree for applying discounts to only one order during common checkout 24_01_2019*/
                var details = PreparePlaceOrderDetailsForOnline(processPaymentRequest, cart,orderTax);
                /*Modified by sree for applying discounts to only one order during common checkout 24_01_2019*/
                ...
                {
                    details.ShippingAddress = _addressService.GetAddressById(processPaymentRequest.CheckoutAddressDetails.ShippingAddressId);
                    // details.BillingAddress = _addressService.GetAddressById(processPaymentRequest.CheckoutAddressDetails.BillingAddressId);
                    details.PickUpInStore = false;
                    // added by Phani for adding cc mails for only online orders on 6/04/2020 start
                    if (processPaymentRequest.PaymentMethodSystemName == "Stripe.Cards")
                    {
                ...
                if (processPaymentResult.Success || isSecondCheckount)
                {
                    ...
                    //Commented By Surakshith to display memberId in mail for register user start on 13-06-2020
                    //  SendNotificationsAndSaveNotes(order);
                    //Commented By Surakshith to display memberId in mail for register user end on 13-06-2020


                    //Added By Surakshith to display memberId in mail for register user strat on 13-06-2020
                    string memberId = MemberId;
                    bool isMember = IsMember;
                    SendNotificationsAndSaveNotesForOnline(order, memberId,isMember);
                    //Added By Surakshith to display memberId in mail for register user end on 13-06-2020
        ...
        public virtual void CheckOrderStatusOrderProcess(Order order, int parentCategoryId)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            switch (order.OrderStatus)
            {
                case OrderStatus.Pending:
                    if (order.PaymentStatus == PaymentStatus.Authorized ||
                        order.PaymentStatus == PaymentStatus.Paid)
                    {
                        if (parentCategoryId == 49)
                            SetOrderStatus(order, OrderStatus.Pending, false);
                        else if (parentCategoryId == 50)
                            SetOrderStatus(order, OrderStatus.Processing, false);
                    }
        ...
        public virtual void ReOrder(Order order)
        {
            ...
            foreach (var orderItem in order.OrderItems)
            {
                _shoppingCartService.AddToCart(order.Customer, orderItem.Product,
                    ...
                    orderItem.Quantity, false, ParentCategoryId: orderItem.ParentCategoryId);
            }
        ...
        /// </summary>
        /// <param name="order">The order</param>
        public virtual void ReOrderForPOS(Order order,Customer customer)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
		...
		//Added  By sree for calculating tax 19_08_2020 start
        protected virtual decimal CalculateTaxPrice(decimal price,decimal taxRate)
        {
            decimal taxPrice = 0;
            taxPrice = price / (1 + (taxRate / 100));
            return Math.Round(taxPrice,4);
        }
				//Added  By sree for calculating tax 19_08_2020 end
        #endregion
    }
}