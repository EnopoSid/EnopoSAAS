...
{
    public partial class FreshOrderService:IFreshOrderService
    {
        #region Fields
        ...
        protected virtual PlaceOrderContainer PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            //load fresh cart
            var fCart = _fCartService.GetFreshCartByCustomerId(details.Customer.Id).Where(x => x.MealDate != null && x.MealTime != null).GroupBy(x => x.MealOrderId).ToArray();
            //var tempFCartShippingItems = _fCartService.GetFreshCartByCustomerId(details.Customer.Id).Where(x => x.MealDate != null && x.MealTime != null);
            ...
            //sub total (incl tax)
            _freshOrderTotalCalulationService.GetFreshShoppingCartSubTotal(details.Cart, fCart, true, out var orderSubTotalDiscountAmount, out var orderSubTotalAppliedDiscounts, out var subTotalWithoutDiscountBase, out var _);
            ...
            //sub total (excl tax)
            _freshOrderTotalCalulationService.GetFreshShoppingCartSubTotal(details.Cart, fCart, false, out orderSubTotalDiscountAmount,
            ...
            //shipping total
            var orderShippingTotalInclTax = _freshOrderTotalCalulationService.GetFreshShoppingCartShippingTotal(details.Cart, fCart, true, out var _, out var shippingTotalDiscounts);
            var orderShippingTotalExclTax = _freshOrderTotalCalulationService.GetFreshShoppingCartShippingTotal(details.Cart, fCart, false);
            ...
            //tax amount
            details.OrderTaxTotal = _freshOrderTotalCalulationService.GetTaxTotal(details.Cart, fCart, out var taxRatesDictionary);
            ...
            var orderTotal = _freshOrderTotalCalulationService.GetFreshShoppingCartTotal(details.Cart, fCart, out var orderDiscountAmount, out var orderAppliedDiscounts, out var appliedGiftCards, out var redeemedRewardPoints, out var redeemedRewardPointsAmount);
        ...
        protected virtual void SendNotificationsAndSaveNotes(Order order)
        {
            ...
            var orderPlacedStoreOwnerNotificationQueuedEmailIds = _workflowMessageService.SendFreshOrderPlacedStoreOwnerNotification(order, _localizationSettings.DefaultAdminLanguageId);
            ...
            var orderPlacedCustomerNotificationQueuedEmailIds = _workflowMessageService
                .SendFreshOrderPlacedCustomerNotification(order, order.CustomerLanguageId);
        ...
        {
            var fcart = _fCartService.GetFreshCartByCustomerId(details.Customer.Id).GroupBy(x => x.MealOrderId).ToArray();

            if (details.Cart[0].ParentCategoryId == 113)
            {
                foreach (var item in fcart)
                {
                    ...
                    {
                        var tempItem = item.Where(x => x.ShoppingCartId == sc.Id).FirstOrDefault();
                        if (tempItem != null)
                        {
                            if (tempItem.MealDate != null && tempItem.MealTime != null)
                            {
                                var scUnitPrice = _freshPriceCalculationService.GetUnitPriceforFreshItem(sc, item);
                                var scSubTotal = _freshPriceCalculationService.GetSubTotalForFreshCart(sc, item, true, out var discountAmount,
                                    out var scDiscounts, out _);
                                var scUnitPriceInclTax =
                                    _freshPriceCalculationService.GetFreshProductPrice(sc.Product, scUnitPrice, true, details.Customer, out var _);
                                var scUnitPriceExclTax =
                                   _freshPriceCalculationService.GetFreshProductPrice(sc.Product, scUnitPrice, false, details.Customer, out _);
                                var scSubTotalInclTax =
                                     _freshPriceCalculationService.GetFreshProductPrice(sc.Product, scSubTotal, true, details.Customer, out _);
                                var scSubTotalExclTax =
                                    _freshPriceCalculationService.GetFreshProductPrice(sc.Product, scSubTotal, false, details.Customer, out _);
                                var discountAmountInclTax =
                                    _freshPriceCalculationService.GetFreshProductPrice(sc.Product, discountAmount, true, details.Customer, out _);
                                var discountAmountExclTax =
                                    _freshPriceCalculationService.GetFreshProductPrice(sc.Product, discountAmount, false, details.Customer, out _);
                                ...
                                FOrderItems fOrderItems = new FOrderItems();
                                fOrderItems.MealOrderId = tempItem.MealOrderId;
                                fOrderItems.ProductId = sc.ProductId;
                                fOrderItems.MealDate = tempItem.MealDate;
                                fOrderItems.MealNo = tempItem.MealNo;
                                fOrderItems.MealPlanId = tempItem.MealPlanId;
                                fOrderItems.MealPrice = tempItem.MealPrice;
                                fOrderItems.MealTime = tempItem.MealTime;
                                fOrderItems.CreatedDate = DateTime.Now;
                                fOrderItems.CustomerId = tempItem.CustomerId;
                                fOrderItems.OrderItemId = orderItem.Id;
                                fOrderItems.OrderId = order.Id;
                                _fOrderItemsService.InsertFreshCart(fOrderItems);
                            ...
                            {
                                if (tempItem.IsReorder != 1)
                                {
                                    _fCartService.RemoveCartByShoppingCartId(tempItem.ShoppingCartId);
                                }
                            }

                            _fCartService.RemoveCartByShoppingCartId(tempItem.Id);
                        }
                        //prices
        ...
        //public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest,IList<ShoppingCartItem> cart)
        //{
        ...
        //                    details.PickupAddress.LastName = "";
        //                }
        ...
        //        {
        //            var freshParentCategoryId = Convert.ToInt32(_localizationService.GetLocaleStringResourceByName("categoryid.fresh").ResourceValue);
        //            if(details.Cart[0].ParentCategoryId == freshParentCategoryId)
        //            {

        //                if (!(bool)processPaymentRequest.CustomValues.Where(x => x.Key == "IsTinPackageSelected").FirstOrDefault().Value)
        //                {
        //                    if ((int)processPaymentRequest.CustomValues.Where(x => x.Key == "PackagingType").FirstOrDefault().Value == Convert.ToInt32(_localizationService.GetLocaleStringResourceByName("fresh.packagingtypes.tincartid").ResourceValue))
        //                        details.OrderTotal = details.OrderTotal - (_freshPackageTypesService.GetPackageTypeById((int)processPaymentRequest.CustomValues.Where(x => x.Key == "PackagingType").FirstOrDefault().Value).Price);                         }
        ...
        //            if (details.Cart[0].ParentCategoryId== freshParentCategoryId)
        //            {
        //                savePackagingType(processPaymentRequest, order, details);
        //                if (processPaymentRequest.CheckoutAddressDetails.PickUpInStore == true)
        //                {
        ...
        //            if (details.Cart[0].ParentCategoryId != freshParentCategoryId)
        //            {
        //                details.OrderTotal = details.OrderTotal - details.OrderShippingTotalInclTax - details.OrderDiscountAmount;
        //                details.OrderShippingTotalInclTax = 0;
        //                details.OrderShippingTotalExclTax = 0;
