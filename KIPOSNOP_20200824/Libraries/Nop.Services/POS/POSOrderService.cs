...
namespace Nop.Services.POS
{
    public class POSOrderService:IPOSOrderService
    {
        ...
        private readonly IAffiliateService _affiliateService = EngineContext.Current.Resolve<IAffiliateService>();
        ...
        private readonly ICustomNumberFormatter _customNumberFormatter = EngineContext.Current.Resolve<ICustomNumberFormatter>();
        ...
        private readonly IPdfService _pdfService = EngineContext.Current.Resolve<IPdfService>();
        ...
        private readonly IPOS_OrdersMappingService _pOS_OrdersMappingService = EngineContext.Current.Resolve<IPOS_OrdersMappingService>();
        private readonly IRepository<Order> _orderRepository = EngineContext.Current.Resolve<IRepository<Order>>();
        ...
        protected virtual PlaceOrderContainer PreparePOSPlaceOrderDetails(ProcessPaymentRequest processPaymentRequest,decimal orderTax)
        {
            ...
            //decimal discountforzerotaxcal = 0;
            //foreach (var disc in orderSubTotalAppliedDiscounts)
            //    if (!_discountService.ContainsDiscount(details.AppliedDiscounts, disc))
            //    {
            //        if (disc.CouponCode.ToUpper().Contains("NQ_"))
            //        {
            ...
            //Added by surakshith for Total Payable amount for Zero payable amount start on 04-06-2020
            ...
            foreach (var disc in orderSubTotalAppliedDiscounts)
                if (!_discountService.ContainsDiscount(details.AppliedDiscounts, disc))
                {
                    if ((disc.CouponCode.ToUpper().Contains("NQ_")))
                    {
                        if(disc.CouponCode == "TakeAwayDiscount_50CEN")
                        {
                    ...
                    {
                        if (disc.CouponCode.ToUpper().Contains("KP_"))
                        {
                        ...
                        {
                            decimal disscountTotal = (details.OrderSubTotalExclTax) * (disc.DiscountPercentage) / 100;
            ...
            /*For Redeem Amount by sree 09_02_2019*/
            //if (details.OrderSubTotalDiscountInclTax != 0 && details.RedeemedRewardPointsAmount != 0)
            //{
            //    details.OrderSubTotalDiscountInclTax = details.OrderSubTotalDiscountInclTax - details.RedeemedRewardPointsAmount;
            //}
            //if (details.OrderSubTotalDiscountExclTax != 0 && details.RedeemedRewardPointsAmount != 0)
            //{
            //    details.OrderSubTotalDiscountExclTax = details.OrderSubTotalDiscountExclTax - details.RedeemedRewardPointsAmount;
            //}
            /*For Redeem Amount by sree 09_02_2019*/
            ...
            if (details.RedeemedRewardPointsAmount != 0)
            {
              ...
              //Added By surakshith to calculate tax based on total payable amount start on 04-08-2020
                decimal taxRate = 0;
                ...
                //decimal taxCalulationCustomForZeroPayableAmount = details.OrderSubTotalExclTax - ActualDiscountAmountExclTax + details.OrderShippingTotalExclTax;
                ...
                details.OrderSubTotalDiscountInclTax = Math.Round(ActualDiscountAmountInclTax, 2);
                details.TaxRates = taxRate + ":" + details.OrderTaxTotal;
               //Added By surakshith to calculate tax based on total payable amount end on 04-08-2020
            }
            //Added By sree for inclusion of tax 07-17-2020 start
            else
            {
                ...
                foreach (var disc in details.AppliedDiscounts)
                    if (!disc.UsePercentage)
                    {
                        if ((disc.CouponCode.ToUpper().Contains("NQ_")))
                        {
                            if (disc.CouponCode == "TakeAwayDiscount_50CEN")
                            {
                                tempdisc = tempdisc + disc.DiscountAmount;
                            }
                        }
                        else
                        {
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            ...
            foreach (var ord in processPaymentRequest.CustomValues)
            {
                var payval = ord.Value;
                if(payval.ToString() == "FreeBill")
                {
                    details.OrderTotal = 0;
                    details.OrderSubTotalInclTax = 0;
                    details.OrderSubTotalExclTax = 0;
            ...
            // DeliveryDate = Convert.ToDateTime(processPaymentRequest.CheckoutAddressDetails.DeliveryDate),
                DeliveryDate = DateTime.Now.AddMinutes(30),
        ...
        protected virtual POS_OrdersMapping SavePOSOrders(ProcessPaymentRequest processPaymentRequest,Order order)
        {
            var getPOSCustomer = _customerService.GetCustomerByGuid((Guid)processPaymentRequest.CustomValues.Where(x => x.Key == "POSUserGuid").FirstOrDefault().Value);
            var posOrder = new POS_OrdersMapping
            {
                OrderId = order.Id,
                OrderGuid = order.OrderGuid,
                POSUserGuid = (Guid)processPaymentRequest.CustomValues.Where(x => x.Key == "POSUserGuid").FirstOrDefault().Value,
                POSUserId = getPOSCustomer.Id,
            ...
            _pOS_OrdersMappingService.InsertPOSOrder(posOrder);
            return posOrder;
        }
        ...
        /// </summary>
        /// <param name="order">Order</param>
        protected virtual void SendNotificationsAndSaveNotes(Order order,ProcessPaymentRequest processPaymentRequest)
        {
            //notes, messages
            ...
            if ((bool)processPaymentRequest.CustomValues.Where(x => x.Key == "IsRegisteredUser").FirstOrDefault().Value == true)
            {
                var currentCustomer = _customerService.GetCustomerByGuid((Guid)processPaymentRequest.CustomValues.Where(x => x.Key == "CustomerGuid").FirstOrDefault().Value);
                var tempOrder = order;
                tempOrder.CustomerId = currentCustomer.Id;
                tempOrder.Customer = currentCustomer;
                var orderPlacedPOSCustomerNotificationQueuedEmailIds = _workflowMessageService
              .SendOrderPlacedCustomerNotification(tempOrder, tempOrder.CustomerLanguageId, orderPlacedAttachmentFilePath, orderPlacedAttachmentFileName);
                if (orderPlacedPOSCustomerNotificationQueuedEmailIds.Any())
                    AddOrderNote(tempOrder, $"\"Order placed\" email (to customer) has been queued. Queued email identifiers: {string.Join(", ", orderPlacedCustomerNotificationQueuedEmailIds)}.");
            }
        ...
        /// </summary>
        /// <param name="order">Order</param>
        protected virtual void SendNotificationsAndSaveNotesForPOS(Order order, ProcessPaymentRequest processPaymentRequest,string MemberId,bool IsMember)
        {
            //Added By Surakshith to display memberId in mail for register user strat on 15-06-2020
            ...
            //Added By Surakshith to display memberId in mail for register user strat on 15-06-2020
            var orderPlacedStoreOwnerNotificationQueuedEmailIds = _workflowMessageService.SendOrderPlacedStoreOwnerNotificationForPOS(order, _localizationSettings.DefaultAdminLanguageId,memberId,isMember);
            //Added By Surakshith to display memberId in mail for register user end on 15-06-2020
            ...
            //Added By Surakshith to display memberId in mail for register user strat on 15-06-2020
            var orderPlacedCustomerNotificationQueuedEmailIds = _workflowMessageService
                .SendOrderPlacedCustomerNotificationForPOS(memberId, isMember,order, order.CustomerLanguageId, orderPlacedAttachmentFilePath, orderPlacedAttachmentFileName);
            //Added By Surakshith to display memberId in mail for register user end on 15-06-2020
            ...
            if ((bool)processPaymentRequest.CustomValues.Where(x => x.Key == "IsRegisteredUser").FirstOrDefault().Value == true)
            {
                ...
                tempOrder.Customer = currentCustomer;
                var orderPlacedPOSCustomerNotificationQueuedEmailIds = _workflowMessageService
              .SendOrderPlacedCustomerNotificationForPOS(memberId, isMember, tempOrder, tempOrder.CustomerLanguageId, orderPlacedAttachmentFilePath, orderPlacedAttachmentFileName);
                if (orderPlacedPOSCustomerNotificationQueuedEmailIds.Any())
                    AddOrderNote(tempOrder, $"\"Order placed\" email (to customer) has been queued. Queued email identifiers: {string.Join(", ", orderPlacedCustomerNotificationQueuedEmailIds)}.");
            }
            var vendors = GetVendorsInOrder(order);
        ...
        /// </summary>
        /// <param name="order">Order</param>
        protected virtual void SendNotificationsAndSaveNotesForKitchen(Order order, ProcessPaymentRequest processPaymentRequest, PlaceOrderContainer details)
        {
            //notes, messages
            AddOrderNote(order, _workContext.OriginalCustomerIfImpersonated != null
                ? $"Order placed by a store owner ('{_workContext.OriginalCustomerIfImpersonated.Email}'. ID = {_workContext.OriginalCustomerIfImpersonated.Id}) impersonating the customer."
            ...
            //send email notifications
            var MailIDForKitchen = details.PickupPoint.ContactAddress; 
            var orderPlacedCentralKitchrnEmailIds = _workflowMessageService.SendOrderPlacedStoreOwnerNotificationForCentralKitchen(order, _localizationSettings.DefaultAdminLanguageId, MailIDForKitchen);
            if (orderPlacedCentralKitchrnEmailIds.Any())
                AddOrderNote(order, $"\"Order placed\" email (to store owner) has been queued. Queued email identifiers: {string.Join(", ", orderPlacedCentralKitchrnEmailIds)}.");
        ...
        /// <param name="order">Order</param>
        protected virtual void MoveShoppingCartItemsToOrderItems(PlaceOrderContainer details, Order order, ProcessPaymentRequest processPaymentRequest)
        {
            foreach (var sc in details.Cart)
            {
                ...
                //Added by Surakshith For Getting Admin Mail bold and bigger size for pos order Items also Start
                ...
                //Added by Surakshith For Getting Admin Mail bold and bigger size for pos order Items also End
            ...
            //if ((bool)processPaymentRequest.CustomValues.Where(x => x.Key == "IsRegisteredUser").FirstOrDefault().Value == true)
            //{
            //    var currentCustomer = _customerService.GetCustomerByGuid((Guid)processPaymentRequest.CustomValues.Where(x => x.Key == "POSUserGuid").FirstOrDefault().Value);
            //    var tempcart=currentCustomer.ShoppingCartItems.Where(sci => sci.ShoppingCartType == ShoppingCartType.ShoppingCart)
            //    .LimitPerStore(processPaymentRequest.StoreId).ToList();
            //    tempcart.ToList().ForEach(sci => _shoppingCartService.DeleteShoppingCartItem(sci, false));
            //}
        }
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                ProcessPaymentResult processPaymentResult = GetProcessPaymentResult(processPaymentRequest, details);
                ...
                else
                {
                    ...
                    foreach (var pickupPoint in pickupPoints)
                    {
                        if (processPaymentRequest.CheckoutAddressDetails.PickupAddressId == Convert.ToInt32(pickupPoint.Id))
                        {
                            ...
                            //Added by Surakshith For Getting Central Kitchen Mail start
                            details.PickupPoint = new PickupPoint
                            {
                                ContactAddress = pickupPoint.ContactAddress
                            };
                            //Added by Surakshith For Getting Central Kitchen Mail End
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    //move shopping cart items to order items
                    MoveShoppingCartItemsToOrderItems(details, order, processPaymentRequest);
                    ...
                    //notifications
                    SendNotificationsAndSaveNotes(order,processPaymentRequest);

                    //notifications
                    //Added by Surakshith For Getting Central Kitchen Mail start
                    //SendNotificationsAndSaveNotesForKitchen(order, processPaymentRequest, details);
                    //Added by Surakshith For Getting Central Kitchen Mail End
                    order.CustomerId = _customerService.GetCustomerByGuid(details.Customer.CustomerGuid).Id;
                    _orderService.UpdateOrder(order);
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult POSPlaceOrder(ProcessPaymentRequest processPaymentRequest,string MemberId,bool IsMember,decimal orderTax)
        {
            if (processPaymentRequest == null)
                throw new ArgumentNullException(nameof(processPaymentRequest));
            ...
            try
            {
                ...
                var details = PreparePOSPlaceOrderDetails(processPaymentRequest,orderTax);
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    string memberId = MemberId;
                    bool isMember = IsMember;
                    SendNotificationsAndSaveNotesForPOS(order, processPaymentRequest,memberId,isMember);
                    //Added By Surakshith to display memberId in mail for register user end on 15-06-2020
