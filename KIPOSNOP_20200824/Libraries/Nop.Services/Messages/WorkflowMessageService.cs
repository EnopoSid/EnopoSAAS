...
namespace Nop.Services.Messages
{
    ...
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        public virtual IList<int> SendOrderPlacedStoreOwnerNotification(Order order, int languageId)
        {
            ...
            _messageTokenProvider.AddOrderTokensAdminEmail(commonTokens, order, languageId);
            ...
            {
                if (order.PickupAddress.LastName.Contains("@"))
                {
                    toEmailaa = Convert.ToString(order.PickupAddress.LastName);
                }
            }
            else if (order.BillingAddress.Company != null)  // added by Phani for adding cc mails for only online orders on 6 / 04 / 2020 start
                            {
                    toEmailaa = order.BillingAddress.Company;
                }
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendOrderPlacedStoreOwnerNotificationForPOS(Order order, int languageId,string MemberId,bool IsMember)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            //Commented By Surakshith to display memberId in mail for register user strat on 13-06-2020
            ...
            _messageTokenProvider.AddOrderTokensAdminEmailForPOS(memberId, isMember, commonTokens, order, languageId);
            ...
            if (order.PickupAddress != null)
            {
                if (order.PickupAddress.LastName != null)
                {
                    if (order.PickupAddress.LastName.Contains("@"))
                    {
                        toEmailaa = Convert.ToString(order.PickupAddress.LastName);
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendOrderPlacedStoreOwnerNotificationForOnline(string MemberId,bool IsMember, Order order, int languageId)
        {
            ...
            string memberId = MemberId;
            bool isMember = IsMember;
            _messageTokenProvider.AddOrderTokensAdminEmailForOnline(memberId,isMember,commonTokens, order, languageId);
            //Added By Surakshith to display memberId in mail for register user end on 13-06-2020
        ...
        /// <summary>
        /// Sends an order placed notification to a store owner For Centrail Kitchen
        /// </summary>
        /// <param name="order">Order instance</param>
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendOrderPlacedStoreOwnerNotificationForCentralKitchen(Order order, int languageId, string EmailIdForKitchen)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var str1 = EmailIdForKitchen;
            //tokens
            var commonTokens = new List<Token>();
            //Added by Phani to remove Price, total, subtotal, shipping, tax and order total on 30-09-2019 start
            _messageTokenProvider.AddOrderTokensForCentralKitchen(commonTokens, str1 , order, languageId);
            //Added by Phani to remove Price, total, subtotal, shipping, tax and order total on 30-09-2019 end
            _messageTokenProvider.AddCustomerTokens(commonTokens, order.Customer);
        ...
        public virtual IList<int> SendOrderProcessedPackersNotification(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.OrderProcessedPackersNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();

            //tokens
            var commonTokens = new List<Token>();
            _messageTokenProvider.AddPackersTokens(commonTokens, order, languageId);
            _messageTokenProvider.AddCustomerTokens(commonTokens, order.Customer);
           // _messageTokenProvider.AddProductTokens(commonTokens,order.OrderItems)
        ...
        /// <summary>
        /// get html body for generate pdf for packer slip
        /// </summary>
        /// <param name="order"></param>
        /// <param name="languageId"></param>
        public string GetPdfPackerTemplateHTML(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.OrderProcessedPackersPdfContent, store.Id);
            if (!messageTemplates.Any())
                return "";
            ...
            var messageTemplate = messageTemplates.Where(x => x.IsActive).FirstOrDefault();
            //email account
            var emailAccount = GetEmailAccountOfMessageTemplate(messageTemplate, languageId);
        ...
        /// <param name="order"></param>
        /// <param name="languageId"></param>
        public string GetPdfInvoiceTemplateHTML(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.OrderUpdatedInvoicePdfContent, store.Id);
            if (!messageTemplates.Any())
                return "";

            //tokens
            var commonTokens = new List<Token>();
            _messageTokenProvider.AddInvoiceTokens(commonTokens, order, languageId);
            _messageTokenProvider.AddCustomerTokens(commonTokens, order.Customer);
            ...
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            ...
            return messageTemplates.Select(messageTemplate =>
            {
                ...
                var toName = $"{order.BillingAddress.FirstName} {order.BillingAddress.LastName}";
                /*Added By Sree for Sending Email to Customer while Choosing PickUp Start 28_12_2018*/
                if (!!order.PickUpInStore)
                {
                   toEmail = order.Customer.Email;
                   toName = order.Customer.Username;
                }
                /*Added By Sree for Sending Email to Customer while Choosing PickUp End 28_12_2018*/
                return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                    attachmentFilePath, attachmentFileName);
            }).ToList();
        }
        ...
        /// <param name="attachmentFileName">Attachment file name. If specified, then this file name will be sent to a recipient. Otherwise, "AttachmentFilePath" name will be used.</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendOrderPlacedCustomerNotificationForPOS(string MemberId,bool IsMember,Order order, int languageId,
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            string memberId = MemberId;
            bool isMember = IsMember;
             _messageTokenProvider.AddOrderTokensForPOS(memberId,isMember,commonTokens, order, languageId);
            //Added By Surakshith to display memberId in mail for register user end on 15-06-2020
        ...
        /// <param name="attachmentFileName">Attachment file name. If specified, then this file name will be sent to a recipient. Otherwise, "AttachmentFilePath" name will be used.</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendOrderPlacedCustomerNotificationForOnline(string MemberId,bool IsMember,Order order, int languageId,
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            string memberId = MemberId;
            bool isMember = IsMember;
            _messageTokenProvider.AddOrderTokensForOnline(memberId,isMember, commonTokens, order, languageId);
            //Added By Surakshith to display memberId in mail for register user strat on 13-06-2020
            ...
            string fromEmail = null, string fromName = null, string subject = null)
        {
            ...
            {
                if (body.Contains("Hi Admin"))
                {
                    if (bcc == null)
                    {
                        bcc = toEmailAddress.Split(';')[1];
                    }
                    else
                    {
                        bcc = bcc + "," + toEmailAddress.Split(';')[1];
                    }
                }
                if (toEmailAddress.Split(';')[0].Contains("del_"))
                {
                    toEmailAddress = "orders@kipos.co";
                }
                else
                {
                    toEmailAddress = toEmailAddress.Split(';')[0];
                }
            }
            // added by Phani for adding cc mails for only online orders on 6/04/2020 end
        ...
        #region Fresh Order WorkFlow

        /// <summary>
        /// Sends an order placed for fresh notification to a store owner
        /// </summary>
        /// <param name="order">Order instance</param>
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendFreshOrderPlacedStoreOwnerNotification(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.OrderFreshPlacedStoreOwnerNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();

            //tokens
            var commonTokens = new List<Token>();
            _messageTokenProvider.AddFreshOrderTokens(commonTokens, order, languageId);
            _messageTokenProvider.AddCustomerTokens(commonTokens, order.Customer);
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendFreshOrderPlacedCustomerNotification(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.OrderFreshPlacedCustomerNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();
        ...
        /// <param name="languageId">Message language identifier</param>
        /// <returns>Queued email identifier</returns>
        public virtual IList<int> SendFreshDepositCustomerNotification(Order order, int languageId)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
            ...
            var messageTemplates = GetActiveMessageTemplates(MessageTemplateSystemNames.FreshOrderTinCartDepositCustomerNotification, store.Id);
            if (!messageTemplates.Any())
                return new List<int>();
