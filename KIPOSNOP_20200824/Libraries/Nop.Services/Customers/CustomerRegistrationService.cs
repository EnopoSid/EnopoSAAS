...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            request.Customer.IsGuestUser = request.IsGuestUser;
            /*Added by surakshith for IsGuestUser End*/
            /*New User Flag during registration*/
            request.Customer.IsNewUser = true;
            /*New User Flag during registration*/
            ...
            //add to 'newuser' role
            var newuserRole = _customerService.GetCustomerRoleBySystemName(NopCustomerDefaults.NewUserRoleName);
            if (newuserRole == null)
                throw new NopException("'newuser' role could not be loaded");
            //request.Customer.CustomerRoles.Add(registeredRole);
            request.Customer.CustomerCustomerRoleMappings.Add(new CustomerCustomerRoleMapping { CustomerRole = newuserRole });

            //add to 'newuser' role0
            var onlineuserRole = _customerService.GetCustomerRoleBySystemName(NopCustomerDefaults.OnlineUserRoleName);
            if (onlineuserRole == null)
                throw new NopException("'newuser' role could not be loaded");
            //request.Customer.CustomerRoles.Add(registeredRole);
            request.Customer.CustomerCustomerRoleMappings.Add(new CustomerCustomerRoleMapping { CustomerRole = onlineuserRole });
