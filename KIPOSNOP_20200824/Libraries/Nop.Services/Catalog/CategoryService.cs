...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        /// <param name="showHidden">A value indicating whether to load hidden records</param>
        /// <returns>Category breadcrumb </returns>
        public virtual List<ProductCategory> GetProductsForGrocer(int GrocerCategoryId)
        {

                //prepare parameters
                var parentCategoryId = _dataProvider.GetInt32Parameter("ParentCategoryId", 49);

                //invoke stored procedure
                var categories = _dbContext.EntityFromSql<ProductCategory>("usp_getProductsByParentCatagery",
                    parentCategoryId).ToList();
            return categories;
        }
        ...
        /// <returns>Products searched</returns>
        public virtual List<Product> GetGrocerProductsBasedOnSearch(string keyWord)
        {

            //prepare parameters
            var KeyWord = _dataProvider.GetStringParameter("KeyWord", keyWord);

            //invoke stored procedure
            var products = _dbContext.EntityFromSql<Product>("usp_searchBasedOnGrocer",
                KeyWord).ToList();
            return products;
        }
        #endregion
