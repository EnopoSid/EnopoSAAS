...
/*Added by sree for getting Products Based on ParentCategory 16_01_2019 Start*/
CREATE PROCEDURE usp_getProductsByParentCatagery
...
BEGIN
	...
	select * from(
	 select *, ROW_NUMBER() over (Partition BY ParentCategoryId ORDER BY PurchaseCount DESC ) AS Ranking from(
	 select pcm.*, c.ParentCategoryId, 
	c.Name as CategoryName,
	(select Name from Category where Id = c.ParentCategoryId) as ParentCategoryName,
	ISNULL(dbo.udf_getProductRatingSum(ProductId),0) as ProductRatingSum,
	[dbo].[udf_getProductPurchaseCount](ProductId) as PurchaseCount
	 from Product_Category_Mapping pcm
	inner join Category c
	on c.Id = pcm.CategoryId
	inner join Product p
	on p.Id = pcm.ProductId
	where p.Published = 1 and p.Deleted = 0 and pcm.CategoryId  in (
    select Id from category where ParentCategoryId in 
	(select Id from Category where ParentCategoryId =  @ParentCategoryId))) tt ) ff where Ranking < 8

END
GO

Create function [dbo].[udf_getProductRatingSum](@productId int)  
returns decimal(7,2)  
as  
begin  
...
SET @Result = (select CASE ApprovedTotalReviews WHEN 0 THEN 0 ELSE ApprovedRatingSum/ApprovedTotalReviews END from [dbo].[Product] where Id = @productId)
...
create function [dbo].[udf_getProductPurchaseCount](@productId int)  
returns int  
as  
begin  
...
SET @Result = (select count(*) from [dbo].[OrderItem] ot inner join Product p on p.Id = ot.ProductId where p.Published = 1 and Deleted = 0 and ProductId = @productId)

    RETURN @Result  

end  
Go
/*Added by sree for getting Products Based on ParentCategory 16_01_2019 End*/

Create procedure usp_searchBasedOnGrocer(
@keyWord varchar(max)
)
as
begin
select prd.* from Product as prd
 join Product_Category_Mapping pcm on pcm.ProductId = prd.Id where CategoryId in (select Id from category where ParentCategoryId in (select Id from Category where ParentCategoryId=49))
 and name like '%'+@keyWord+'%' and Published=1 and Deleted=0
 end
 Go